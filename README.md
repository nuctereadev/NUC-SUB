...
- یک **لینک** و **توکن** نمایش داده می‌شود (همچنین در `/opt/nuc-sub/install-result.txt`)
- وب‌پنل روی پورت پیش‌فرض `8080` اجرا می‌شود (قابل تغییر با `XUI_SUB_PORT=XXXX`)
...
- پورت پیش‌فرض: `8080`
- ورود با توکنی که هنگام نصب تولید می‌شود
